...
class BTLegoController(BTLegoDevice):
	# 0:	Don't debug
	# 1:	Print weird stuff
...
# You're gonna get these.  Don't know why I even let you choose?
					...
					BTLegoController.dp("INVALID Subscription option:"+subscription)

		else:
			BTLegoController.dp("NOT CONNECTED.  Not setting port subscriptions",2)
	...
	async def device_events(self, sender, data):
		...
		if bt_message['error']:
			BTLegoController.dp(msg_prefix+"ERR:"+bt_message['readable'])
			...
			if BTLego.message_type_str[bt_message['type']] == 'port_input_format_single':
				if BTLegoController.DEBUG >= 2:
					...
					BTLegoController.dp(msg_prefix+msg+port_text+", mode "+str(bt_message['mode']), 2)
			...
			# Sent on connect, without request
				...
				if event == 'attached':
					...
					if bt_message['port'] in self.port_data:
						BTLegoController.dp(msg_prefix+"Re-attached "+dev+" on port "+str(bt_message['port']),2)
						self.port_data[bt_message['port']]['status'] = bt_message['event']
					else:
						BTLegoController.dp(msg_prefix+"Attached "+dev+" on port "+str(bt_message['port']),2)
						self._init_port_data(bt_message['port'], bt_message['io_type_id'])

				elif event == 'detached':
					BTLegoController.dp(msg_prefix+"Detached "+dev+" on port "+str(bt_message['port']),2)
					self.port_data[bt_message['port']]['status'] = 0x0 # io_event_type_str

				else:
					BTLegoController.dp(msg_prefix+"HubAttachedIO: "+bt_message['readable'],1)

			elif BTLego.message_type_str[bt_message['type']] == 'port_value_single':
				if not bt_message['port'] in self.port_data:
					BTLegoController.dp(msg_prefix+"WARN: Received data for unconfigured port "+str(bt_message['port'])+':'+bt_message['readable'])
				else:
					...
					if pd['name'] == 'Powered Up Handset Buttons':
						...
						if BTLegoController.DEBUG >= 2:
							BTLegoController.dp(msg_prefix+"Data on "+self.port_data[bt_message['port']]['name']+" port"+":"+" ".join(hex(n) for n in data),2)
				...
				if not BTLego.hub_property_op_str[bt_message['operation']] == 'Update':
					...
					BTLegoController.dp(msg_prefix+"ERR NOT UPDATE: "+bt_message['readable'])

				else:
					if not bt_message['property'] in BTLego.hub_property_str:
						BTLegoController.dp(msg_prefix+"Unknown property "+bt_message['readable'])
					else:
						if BTLego.hub_property_str[bt_message['property']] == 'Button':
							if bt_message['value']:
								BTLegoController.dp(msg_prefix+"Bluetooth button pressed!",2)
						...
						# The app seems to be able to subscribe to Battery Voltage and get it sent constantly
							...
							BTLegoController.dp(msg_prefix+"Battery is at "+str(bt_message['value'])+"%",2)
							self.message_queue.put(('info','batt',bt_message['value']))

						elif BTLego.hub_property_str[bt_message['property']] == 'Advertising Name':
							BTLegoController.dp(msg_prefix+"Advertising as \""+str(bt_message['value'])+"\"",2)
							pass

						else:
							BTLegoController.dp(msg_prefix+bt_message['readable'],2)
				...
				BTLegoController.dp(msg_prefix+" "+bt_message['readable'],3)
				pass

			elif BTLego.message_type_str[bt_message['type']] == 'hub_alerts':
				# Ignore "status OK" messages
				if bt_message['status'] == True:
					BTLegoController.dp(msg_prefix+"ALERT! "+bt_message['alert_type_str']+" - "+bt_message['operation_str'])
				...
				BTLegoController.dp(msg_prefix+"-?- "+bt_message['readable'],1)

		BTLegoController.dp("Draining for: "+bt_message['readable'],3)
	...
	def decode_button_data(self, port, data):
		if len(data) != 1:
			BTLegoController.dp(self.system_type+" UNKNOWN BUTTON DATA, WEIRD LENGTH OF "+str(len(data))+":"+" ".join(hex(n) for n in data))
		...
		if button_id == 0x0:
			...
			BTLegoController.dp(self.system_type+" Unknown button "+hex(button_id))
	...
	def decode_bt_rssi_data(self, data):
		...
		BTLegoController.dp("RSSI: "+str(rssi8))
	...
	def decode_voltage_data(self,data):
		...
		BTLegoController.dp("Voltage: "+str(volts16)+ " millivolts")

	# ---- Random stuff ----

	def dp(pstr, level=1):
		if BTLegoController.DEBUG:
			if BTLegoController.DEBUG >= level:
				print(pstr)
	...
	async def write_mode_data_RGB_color(self, port, color):
		...
		# BTLegoController.dp(self.system_type+" Debug RGB Write"+" ".join(hex(n) for n in payload))
